...
if [ -z "$JDK" -a -s "$HOME/.GoLand2018.1/config/goland.jdk" ]; then
  USER_JRE=`"$CAT" $HOME/.GoLand2018.1/config/goland.jdk`
  if [ ! -d "$USER_JRE" ]; then
...
elif [ -r "$HOME/.GoLand2018.1/config/goland$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.GoLand2018.1/config/goland$BITS.vmoptions"
...
"$JAVA_BIN" \
  ...
  -Didea.paths.selector=GoLand2018.1 \
